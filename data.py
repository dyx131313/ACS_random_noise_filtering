...
class dataset:
    ...
    def gen_data(self, start=1, stop=20, points=20, expression="x"):
...
# 生成并保存数据
train_dataset = [
    dataset(expression="np.sin(x/5)", label="sin"),
    dataset(expression="np.cos(x/5)", label="cos"),
    dataset(expression="x", label="liner"),
    dataset(expression="(x * x - 2 * x)/5", label="quadratic"),
    dataset(expression="0.2 / x", label="verse"),
    dataset(expression="0.4 / x", label="verse2"),
    dataset(expression="2 / x", label="verse3")
]
# # 示例：从CSV文件中加载数据
# train_dataset = [
