    ...
    def train(self):
        for epoch in range(self.num_epochs):
            for data in train_dataset:
                ...
                X = torch.tensor(combined_input, dtype=torch.float32).unsqueeze(1).to(self.device)
                y = torch.tensor(points_diff[1:], dtype=torch.float32).to(self.device)
    ...
    def predict(self):
        ...
        with torch.no_grad():
            # for data in train_dataset:
                ...
                combined_input = np.concatenate((points_diff_[:-1], points_shift_x_repeated[:-1], points_norm_repeated[:-1]), axis=1)
                X = torch.tensor(combined_input, dtype=torch.float32).unsqueeze(1)
